#include <bits/stdc++.h> //https://neps.academy/problem/777
using namespace std;
...
int seg_query(int i, int l, int r, int ql, int qr)
{
  ...
  if (l > qr || r < ql)
    return 0;
  int mid = (l + r) / 2;
  return seg_query(2 * i, l, mid, ql, qr) | seg_query((2 * i) + 1, mid + 1, r, ql, qr);
}
...
void init()
{
  ...
  decompose(0, 0);
  for (int i = 0; i < n; i++)
    seg_update(1, 0, n - 1, pos[i], pow(2, color[i]));
}
...
signed main()
{
  ...
  while (q--)
  {
    ...
    if (t == 1)
    {
      ...
      cin >> a >> b;
      a--;
      seg_update(1, 0, n - 1, pos[a], pow(2, b));
    }
    else
    {
      int a, b;
