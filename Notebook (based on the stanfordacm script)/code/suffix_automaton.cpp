...
// suffix automaton
// definicao: um suffix automaton de uma string s e um automato finito deterministico
...
// e uma aresta representa uma transicao (um caractere a mais em relacao ao estado(suffixo) atual)
// t0 -> estado inicial(string vazia), e todos os demais estados podem ser alcancados a partir de t0
...
// pois, qualquer caminho comecando do estado t0 corresponde a uma substring de s

// conceitos:

// 1 - endpos
// seja t uma substring de s, endpos(t) eh o conjunto de todas os indices(posicoes)
...
// info 1: se duas substrings u e w u.size() <= w.size(), se u eh um sufixo de w, logo endpos(u) esta contido em endpos(w)
// info 2: se duas substrings u e w u.size() <= w.size(), se u nao eh um sufixo de w, logo nao existe interseccao entre endpos(u) e endpos(w)
...
// com isso um suffix_link(v) corresponde ao maior suffix de w que esta em outra classe de equivalencia pelos endpos
...
// 3 - complexidade O(n) ou O(n log k), se usarmos uma map para guardar as transicoes partindo de um estado
...
// com isso, o numero de substrings distintas eh o numero de caminhos diferentes que comecam de t0
...
// se considerarmos as transicoes sempre indo do menor char para o maior durante o percurso
