...
// resolve um problema de programação linear para maximizar uma função  c[0] * x[0] + c[1] * x[1] + ... <= b (c[i] eh o coeficiente do i-esimo cara na função objetiva)
// sujeito a restrições que tem a seguinte forma:
// a[0] * x[0] + a[1] * x[1] + ... <= b, (a[i] eh o coeficiente)
// ai todas as restrições sao passadas nos vectors a e b
// complexidade: 2^n, mas na prática pode ser melhor do que isso
struct lp_solver
{
  int m, n;
  vector<int> N, B;
